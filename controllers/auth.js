...
let registerUser = (fullName, mobileNumber, email, password, sms1, sms2) => {
	return new Promise((res, rej) => {
		...
		return Users.find({ email: email })
			.then(user => {
				...
				let newUser = { fullName, mobileNumber, email, sms1, sms2 };
...
let createToken = user => {
	return jwt.sign(_.omit(user.toObject(), 'password'), config.secret, {
		expiresIn: '36000s' //lower value for testing
	});
};
